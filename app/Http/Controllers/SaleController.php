...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                // $nestedData['options'] .=
                //     '<li>
                //     <button type="button" class="send-sms btn btn-link" data-id = "' . $sale->id . '" data-customer_id="' . $sale->customer_id . '" data-reference_no="' . $nestedData['reference_no'] . '" data-sale_status="' . $sale->sale_status . '" data-payment_status="' . $sale->payment_status . "\" \u{00A0}data-toggle=\"modal\" data-target=\"#send-sms\"><i class=\"fa fa-envelope\"></i> " . __('db.Send SMS') . '</button>
                // </li>';
                ...
                $nestedData['taxdetails'] = [
                    (float) $sale->total_extra_tax,
                    (float) $sale->total_further_tax,
                    (float) $sale->total_fed_payable,
                ];
    ...
    public function store(StoreSaleRequest $request)
    {
        ...
        $mail_data['order_tax'] = $lims_sale_data->order_tax ?? 0;
        $mail_data['order_tax_rate'] = $lims_sale_data->order_tax_rate ?? 0;
        ...
        foreach ($product_id as $i => $id) {
            ...
            // Extra Columns For FBR Product tos tore FBR Values in Product Sale Table
            $product_sale['value_sales_excluding_st'] = (float) $total[$i] - $tax[$i];
            $product_sale['fixed_notified_value_or_retail_price'] = (float) $lims_product_data->fixed_notified_value_or_retail_price;
            $product_sale['sales_tax_withheld_at_source'] = (float) $lims_product_data->sales_tax_withheld_at_source;
            $product_sale['extra_tax'] = (float) $lims_product_data->extra_tax;
            $product_sale['further_tax'] = (float) $lims_product_data->further_tax;
            $product_sale['sro_schedule_no'] = $lims_product_data->sro_schedule_no ?? '';
            $product_sale['fed_payable'] = (float) $lims_product_data->fed_payable;
            $product_sale['sale_type'] = $scenarioType;
            $product_sale['sro_item_serial_no'] = $lims_product_data->sroItemSerialNo ?? '';

            $total_product_tax += $tax[$i];

            $total_extra_tax += $lims_product_data->extra_tax;
            $total_further_tax += $lims_product_data->further_tax;
            $total_fed_payable += $lims_product_data->fed_payable;

            // FBR Items PayLoad Prepration
            $fbrProductsData[] = [
                'hsCode' => $lims_product_data->hs_code,
                'productDescription' => $lims_product_data->name,
                'rate' => str_ends_with($tax_rate[$i], '%') ? (int) $tax_rate[$i] : (int) $tax_rate[$i] . '%',
                'uoM' => $lims_sale_unit_data->unit_code,
                'quantity' => (int) $qty[$i],
                'totalValues' => (float) $total[$i],
                'valueSalesExcludingST' => (float) $total[$i] - $tax[$i],
                'fixedNotifiedValueOrRetailPrice' => (float) $lims_product_data->fixed_notified_value_or_retail_price,
                'salesTaxApplicable' => (float) $tax[$i],
                'salesTaxWithheldAtSource' => (float) $lims_product_data->sales_tax_withheld_at_source,
                'extraTax' => (float) $lims_product_data->extra_tax,
                'furtherTax' => (float) $lims_product_data->further_tax,
                'sroScheduleNo' => $lims_product_data->sro_schedule_no ?? '',
                'fedPayable' => (float) $lims_product_data->fed_payable,
                'discount' => (float) $discount[$i],
                'saleType' => $scenarioType,
                'sroItemSerialNo' => $lims_product_data->sroItemSerialNo ?? '',
            ];

            Product_Sale::create($product_sale);
        }

        // Sales Table taxes Update Start
        $grand_total = ($total_extra_tax + $total_further_tax + $total_fed_payable);
        DB::table('sales')
            ->where('id', $lims_sale_data->id)
            ->update([
                'total_extra_tax' => (float) $total_extra_tax,
                'total_further_tax' => (float) $total_further_tax,
                'total_fed_payable' => (float) $total_fed_payable,
                'grand_total' => DB::raw("grand_total + $grand_total")  // add to existing value
            ]);
        ...
        if ($mail_data['email'] && $data['sale_status'] == 1 && $mail_setting) {
            ...
            try {
                ...
                $message = ' Sale created successfully. Please setup your mail setting to send mail.';
            }
        }
        ...
        // fbr posting start
        if ($data['fbr_posting'] == 1) {
            $customer_data = Customer::find($data['customer_id']);
            $business_configurations = getBusinessConfigurations();

            $fbrPayload = [
                'invoiceType' => 'Sale Invoice',
                'invoiceDate' => date('Y-m-d', strtotime($data['created_at'])),
            ...
            if (!$validation['success']) {
                ...
                $message = 'Sale created successfully. FBR Validation Failed. Please check FBR Validation Status.';
            } else {
            ...
            if ($validationPassed) {
                ...
                if (!$posting['success']) {
                    ...
                    $message = 'Sale created successfully. FBR Validation Passed but Posting Failed. Please check FBR Validation Status.';
                }
                if ($posting['success']) {
                    // $qrData     = $posting['data']['invoiceNumber'];
                    // $qrFileName = 'qr_' . $lims_sale_data->reference_no . '.png';
                    ...
                    // $result->save(public_path('images/fbrqr_codes'), $qrFileName);

                    // $path = 'fbrqr_codes/' . $qrFileName;

                    // Assuming $lims_sale_data is created and has an id
                    $q = DB::table('sales')
                        ->where('id', $lims_sale_data->id)
                        ->update([
                    ...
                    $message = 'Sale created successfully. FBR Validation Passed and Posting Successful.';
                }
            }
            if (!empty($fbrErrors)) {
                foreach ($fbrErrors as $err) {
                    FbrPostError::logError($err);
                    Log::info('FBR Error. Please check FBR Validation Status.', [
                        'error' => $err
                    ]);
                }
            }
    ...
    public function getProduct($id)
    {
        ...
        $lims_product_with_variant_warehouse_data = $query
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) {
            if (!isset($product_warehouse->is_imei)) {
                if (isset($product_warehouse->imei_number))
                    continue;
            }
        ...
        if ($key == 'category') {
            $query = $query
                ...
                ->where(function ($query) use ($cat_or_brand_id) {
                    $query
                        ->where('products.category_id', $cat_or_brand_id)
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $productArray = [
            ...
            $batch->batch_no ?? '',  // 22
            $product->hs_code,
            $product->fixed_notified_value_or_retail_price,
            $product->sales_tax_withheld_at_source,
            $product->extra_tax,
            $product->further_tax,
            $product->fed_payable,
            $product->sro_schedule_no,
            $product->sro_item_serial_no,
            $product->is_fbr_invoice_product,
        ];
    ...
    public function submitOrderRequest($data, $amount)
    {
        ...
        // $middle_name = "Coders";
        $last_name = $data->name;
        $email_address = $data->email ? $data->email : 'hammadraja2003@gmail.com';
        if ($APP_ENVIROMENT == 'sandbox') {
        ...
        if (preg_match('/^\d{13}$/', $search)) {
            ...
            $product = Product::where('is_embeded', true)
                ->where(function ($q) use ($product_embed_code) {
                    $q
                        ->where('code', 'like', $product_embed_code . '%')
