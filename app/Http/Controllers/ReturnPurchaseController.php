        ...
        if (!empty($returnss)) {
            foreach ($returnss as $key => $returns) {
                ...
                $nestedData['return'] = array(
                    ...
                    ' "' . preg_replace('/[\n\r]/', '<br>', $returns->warehouse->address) . '"',
                    ' "' . $supplier->name . '"',
                    ' "' . $supplier->company_name . '"',
                    ' "' . $supplier->email . '"',
                    ' "' . $supplier->phone_number . '"',
                    ' "' . preg_replace('/[\n\r]/', '<br>', $supplier->address) . '"',
                    ' "' . $supplier->city . '"',
                    ' "' . $returns->id . '"',
                    ' "' . $returns->total_tax . '"',
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($lims_product_return_data as $key => $product_return_data) {
            ...
            if ($product_return_data->purchase_unit_id != 0) {
                ...
                ProductPurchase::where('purchase_id', $lims_return_data->purchase_id)->where('product_id', $product_return_data->product_id)->update(['return_qty' => $quantity]);
            }
        ...
        foreach ($return_id as $id) {
            ...
            foreach ($lims_product_return_data as $key => $product_return_data) {
                ...
                if ($product_return_data->purchase_unit_id != 0) {
                    ...
                    ProductPurchase::where('purchase_id', $lims_return_data->purchase_id)->where('product_id', $product_return_data->product_id)->update(['return_qty' => 0]);
                }
            }
