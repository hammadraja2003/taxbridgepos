...
class PayrollController extends Controller
{
    ...
    public function getEmployeesByWarehouse(Request $request)
    {
        ...
        // Step 1: Get user IDs from MASTER DB
        $userIds = \App\Models\User::where('warehouse_id', $warehouse_id)
            ->pluck('id')
            ->toArray();

        // Step 2: Get employees from TENANT DB
        $employees = \App\Models\Employee::whereIn('user_id', $userIds)
            ->where('is_active', true)
            ->get(['id', 'name']);
