    ...
    <section>
        <div class="container-fluid">
            <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#createModal"><i
                    class="dripicons-plus"></i> {{__('db.Count Stock')}} </button>
        </div>
                    ...
                    @foreach($lims_stock_count_all as $key => $stock_count)
                                    ...
                                    <tr>
                                        ...
                                        @if($stock_count->type == 'full')
                                            ...
                                            <td>
                                                <div class="badge badge-primary">{{__('db.Full')}}</div>
                                            </td>
                                        @else
                                            @php $type = __('db.Partial') @endphp
                                            <td>
                                                <div class="badge badge-info">{{__('db.Partial')}}</div>
                                            </td>
                                        @endif
                                        <td class="text-center">
                                            <a download href="{{'stock_count/' . $stock_count->initial_file}}"
                                                title="{{__('db.Download')}}"><i class="dripicons-copy"></i></a>
                                        </td>
                                        <td class="text-center">
                                            @if($stock_count->final_file)
                                                <a download href="{{'stock_count/' . $stock_count->final_file}}"
                                                    title="{{__('db.Download')}}"><i class="dripicons-copy"></i></a>
                                            @endif
                                        </td>
                                        <td>
                                            ...
                                            @else
                                                <div style="cursor: pointer;" class="badge badge-primary finalize"
                                                    data-id="{{$stock_count->id}}">{{__('db.Finalize')}}
                                                </div>
                                            @endif
                                        </td>
                                    </tr>
    ...
    <div id="finalizeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"
        ...
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="italic">
                        <small>{{__('db.The field labels marked with * are required input fields')}}.<strong>{{__('db.You just need to update the Counted column in the initial file')}}</strong>
                        </small>
                    </p>
                    <div class="form-group">
